...
import {
  ...
  type BlackReportPeriod,
  type GenerateBlackReportInput,
  BLACK_REPORT_TYPE_GROUPS,
} from './constants';

interface Props {
  open: boolean;
  onClose: () => void;
  onGenerate: (input: GenerateBlackReportInput) => void;
  isPending?: boolean;
}

const PERIODS: { key: BlackReportPeriod; label: string }[] = [
...
export function GenerateModal({ open, onClose, onGenerate, isPending = false }: Props): React.ReactElement | null {
  const [reportType, setReportType] = useState<BlackReportType | ''>('black_closing_stock');
  const [period, setPeriod] = useState<BlackReportPeriod>('DAILY');
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-[500px] flex-col overflow-hidden rounded-xl bg-background shadow-2xl border border-border">
        {/* Header */}
        <div className="flex items-start justify-between border-b px-6 py-5">
          <div>
            <h3 className="text-base font-bold text-foreground">Generate Black Stock Report</h3>
            <p className="mt-0.5 text-sm text-muted-foreground">Select report category, time period, and location</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-6">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-bold text-foreground">
              Report Type{' '}
              <span className="ml-1 rounded bg-red-50 dark:bg-red-950/40 px-1.5 py-0.5 text-[10px] font-semibold text-red-600 dark:text-red-400">
                Required
              </span>
            </label>
            <FormSelect
              value={reportType}
              onChange={(v) => setReportType(v as BlackReportType)}
              placeholder="— Select report type —"
              options={BLACK_REPORT_TYPE_GROUPS.flatMap((grp) =>
                grp.options.map((o) => ({
                  value: o.value,
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              Choose an inventory, sales markup, or commission report to aggregate.
            </p>
          </div>
